...
from benchmark.types import Condition, GuessEvaluation, GuessStatus, HistoryEntry
...
def validate_guess(
    ...
    if len(normalized) != 5 or not normalized.isascii() or not normalized.isalpha():
        return GuessEvaluation(raw_guess, normalized, GuessStatus.FORMAT_ERROR)
    if normalized not in legal_guesses:
        subcode = "OUTSIDE_DYNAMIC_POOL" if condition is Condition.DYNAMIC_256 else None
        return GuessEvaluation(raw_guess, normalized, GuessStatus.LEXICON_ERROR, subcode)
