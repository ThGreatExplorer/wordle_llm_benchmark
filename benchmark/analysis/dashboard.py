...
<div class="controls"><label>Model<select id="model"></select></label><label>Condition<select id="condition"></select></label><label>Mode<select id="mode"></select></label><label>Metric<select id="metric"></select></label></div>
...
const metricNames=['solve_at_6','mean_round_score','played_guesses_per_game','initial_action_valid_at_1','initial_action_valid_at_3','constraint_consistent_at_1','constraint_consistent_at_3','strict_valid_at_1','strict_valid_at_3','ig_efficiency','search_regret','ranking_regret','total_regret','repair_success_rate','forfeits_per_game','forfeit_rate','repeat_rate','constraint_age'];
...
options('mode',[...new Set(D.metrics.map(x=>x.game_mode))]);
$('metric').innerHTML=metricNames.map(v=>`<option>${v}</option>`).join('');
function filtered(rows){return rows.filter(x=>($('model').value==='*'||x.model_key===$('model').value)&&($('condition').value==='*'||x.condition===$('condition').value)&&($('mode').value==='*'||x.game_mode===$('mode').value))}
